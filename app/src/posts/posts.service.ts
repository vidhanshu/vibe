...
const createPost = async ({
  ...
  try {
    ...
    if (res.status !== 201) {
      ...
      if (uploadedFiles.length)
        await fileService.deleteFiles(uploadedFiles.map(({ key }) => key));
...
const deletePost = async (postId: string): NSCommon.Response<null> => {
  try {
...
const getPosts = async ({
  ...
  username?: string;
  page?: number;
}): NSCommon.Response<NSCommon.PaginatedResponse<NSPost.DetailedPost>> => {
  try {
    const sp = new URLSearchParams();
    if (page) sp.append("page", page.toString());
    if (username) sp.append("username", username);
    if (tag) sp.append("tag", tag);
    const res = await api.get(`/posts?${sp.toString()}&limit=6`);
    return { data: res.data };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  } catch (error: any) {
    return {
...
//
const likeUnLike = async (postId: string): NSCommon.Response<NSPost.Like> => {
  try {
